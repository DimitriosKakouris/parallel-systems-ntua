...
int main(int argc, char ** argv) {
	...
	*/
    ...
    for (t=0;t<T;t++) {
        ...
        if (south_flag){
    		MPI_Isend(
				...
				MPI_COMM_WORLD, // community
				// &req
				&reqs_prev_sent[count_neighbours_prev]
			);
			// count_neighbours += 1;
			MPI_Irecv(
                ...
                0, // receive tag
                MPI_COMM_WORLD, // community
				&reqs_prev_received[count_neighbours_prev]
			);
			count_neighbours_prev+=1;
        }
		...
		MPI_Waitall(count_neighbours_prev, reqs_prev_received, MPI_STATUSES_IGNORE);

        // end communication with previous

        swap=u_previous;
		u_previous=u_current;
		u_current=swap;

        // start communication with current
        MPI_Request reqs_curr_sent[4];
		MPI_Request reqs_curr_received[4];
		// MPI_Status stats[4];
		int count_neighbours_curr = 0;
        ...
        if (north_flag){
			...
			MPI_Irecv(
                ...
                0, // receive tag
                MPI_COMM_WORLD, // community
				&reqs_curr_received[count_neighbours_curr]
			);
			count_neighbours_curr+=1;
        }
        ...
        MPI_Waitall(count_neighbours_curr,reqs_curr_received,MPI_STATUSES_IGNORE);


        count_neighbours_curr=0;
        ...
        tcomp+=(tcf.tv_sec-tcs.tv_sec)+(tcf.tv_usec-tcs.tv_usec)*0.000001;
         ...
         if (south_flag){
    		MPI_Isend(
				...
				MPI_COMM_WORLD, // community
				// &req
				&reqs_curr_sent[count_neighbours_curr]
			);
			count_neighbours_curr+=1;
        }
        // communicate with east neighbour
        ...
        MPI_Waitall(count_neighbours_curr, reqs_curr_sent, MPI_STATUS_IGNORE);
        MPI_Waitall(count_neighbours_prev, reqs_prev_sent, MPI_STATUS_IGNORE);
    }
	...
	printf("GaussSeiderSOR X %d Y %d Px %d Py %d Iter %d ComputationTime %lf TotalTime %lf midpoint %lf\n",global[0],global[1],grid[0],grid[1],t,comp_time,total_time,U[global[0]/2][global[1]/2]);
