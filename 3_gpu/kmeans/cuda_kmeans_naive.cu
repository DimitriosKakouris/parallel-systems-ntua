                ...
                int blockSize)
{
 ...
 /* TODO: Calculate Grid size, e.g. number of blocks. */
    ...
    checkCuda(cudaMemcpy(deviceObjects, objects,numObjs*numCoords*sizeof(double), cudaMemcpyHostToDevice));
	...
	fprintf(fp, "%s,%d,%lf,%lf,%lf,%lf\n", "Naive", blockSize, timing/loop, timer_min, timer_max,timing);
    #endif
    
    #ifdef TIMING_ANALYSIS
    fprintf(fp, "%s,%d,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf \n", "Naive", blockSize, timing/loop, timer_min, timer_max,timing,1000*gpu_cpu_time_total, 1000*cpu_gpu_time_total, 1000*gpu_time_total,1000*cpu_time_total);
    #endif
